...
const Chat = () => {
  ...
  // Get the customer's details (the other participant)
  const getCustomerDetails = () => {
    if (!conversation || !user?.id) return null;
    
    // Find the participant that is not the current user (stylist)
    const customerParticipant = conversation.participant_details.find(
  ...
  useEffect(() => {
    ...
    const handleMessage = (msg: any) => {
      ...
      console.log("Received message in stylist chat page:", msg.type);
    };
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      >
        ...
        >
          <Avatar
            uri={customerDetails?.profile_image_url || temporaryContact?.profile_image_url}
            width={32}
            height={32}
          />
          <Heading style={{ color: Colors.primaryLight }}>
            {customerDetails?.name || temporaryContact?.name || "Chat"}
          </Heading>
        </View>
